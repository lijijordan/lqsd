...
public class BaseFormController implements ServletContextAware {
	...
	/**
	 * Add by wangsulong，用于展示登录信息
	 * 
	 * @param model
	 * @param request
	 */
	@ModelAttribute
	public void loginInfo(Model model, HttpServletRequest request) {
		Staff staff = (Staff) request.getSession(true).getAttribute(
				"security_staff");
		if (staff == null) {
			staff = new Staff();
			staff.setName("非法用户");
		}
		model.addAttribute("loginUser", staff);
	}
